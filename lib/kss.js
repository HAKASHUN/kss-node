...
var walk = require('./walk.js'),
	...
	KssAngular = require('./kss_angular.js'),
...
parseChunk = function(data, input, options) {
    ...
    //angular
    currSection = processAngular(paragraphs, options, currSection);
...
processAngular = function(paragraphs, options, sectionData) {
  var removeIndex = 'nope';

  paragraphs.map(function(paragraph, index) {
    if (hasPrefix(paragraph, options, 'Angular')) {
      sectionData.angular = paragraph.replace(/^\s*?[a-z ]+\:\s+?/i, '');
      paragraph = '';
      removeIndex = index;
    }
    return paragraph;
